...
{
  { WINDOW_CreateIndirect, "", ID_WINDOW_0, 0, 0, 240, 450, 0, 0x64, 0 },
};
...
static void _cbDialog_font(WM_MESSAGE * pMsg)
{
  ...
  {
    case WM_CREATE:
      hItem = IMAGE_CreateEx(190, 10, 40, 70, pMsg->hWin, WM_CF_SHOW, 0, GUI_ID_IMAGE_ARABIC);
      IMAGE_SetBitmap(hItem, &bmarabic);
      hItem = IMAGE_CreateEx(190, 10, 40, 70, pMsg->hWin, WM_CF_SHOW, 0, GUI_ID_IMAGE_ITALIAN);
      IMAGE_SetBitmap(hItem, &bmitalian);
      hItem = IMAGE_CreateEx(190, 10, 40, 70, pMsg->hWin, WM_CF_SHOW, 0, GUI_ID_IMAGE_CHINESE);
      IMAGE_SetBitmap(hItem, &bmchinese);
      hItem = IMAGE_CreateEx(190, 10, 40, 70, pMsg->hWin, WM_CF_SHOW, 0, GUI_ID_IMAGE_FRENCH);
      IMAGE_SetBitmap(hItem, &bmfrench);
      hItem = IMAGE_CreateEx(190, 10, 40, 70, pMsg->hWin, WM_CF_SHOW, 0, GUI_ID_IMAGE_ENGLISH);
      IMAGE_SetBitmap(hItem, &bmenglish);

      hItem = IMAGE_CreateEx(185, 5, 55, 70, pMsg->hWin, WM_CF_SHOW, 0, GUI_ID_IMAGE_WORLD);
      IMAGE_SetGIF(hItem, _acworld, sizeof(_acworld));

      GUI_UC_SetEncodeUTF8();
      /* English */
      hItem = TEXT_CreateEx(55, 145, 380, 60, pMsg->hWin, WM_CF_SHOW, 0, GUI_ID_TEXT_ENGLISH1, "welcome");
      TEXT_SetTextColor(hItem, 0xFF5E1602);
      TEXT_SetFont(hItem, &GUI_Fonteng50);

      /* French */
      hItem = TEXT_CreateEx(80, 180, 260, 40, pMsg->hWin, WM_CF_SHOW, 0, GUI_ID_TEXT_FRENCH1, "bienvenue");
      TEXT_SetTextColor(hItem, 0x6B6704);
      TEXT_SetFont(hItem, &GUI_Fonteng40);

      hItem = TEXT_CreateEx(125, 130, 115, 25, pMsg->hWin, WM_CF_SHOW, 0, GUI_ID_TEXT_FRENCH2, "bienvenue");
      TEXT_SetTextColor(hItem, 0xF89D2D);
      TEXT_SetFont(hItem, &GUI_Fonteng25);

      /* Chinese */
      hItem = TEXT_CreateEx(75, 210, 160, 60, pMsg->hWin, WM_CF_SHOW, 0, GUI_ID_TEXT_CHINESE1, _apStrings[0]);
      TEXT_SetTextColor(hItem, 0xF94508);
      TEXT_SetFont(hItem, &GUI_FontchineseI60B);

      hItem = TEXT_CreateEx(80, 60, 120, 40, pMsg->hWin, WM_CF_SHOW, 0, GUI_ID_TEXT_CHINESE2, _apStrings[0]);
      TEXT_SetTextColor(hItem, 0x08CFF9);
      TEXT_SetFont(hItem, &GUI_Fontchinese40);

      hItem = TEXT_CreateEx(15, 160, 40, 25, pMsg->hWin, WM_CF_SHOW, 0, GUI_ID_TEXT_CHINESE3, _apStrings[0]);
      TEXT_SetTextColor(hItem, 0x08CFF9);

      TEXT_SetFont(hItem, &GUI_Fontchinese25B);

      /* Italian */
      hItem = TEXT_CreateEx(10, 90, 270, 60, pMsg->hWin, WM_CF_SHOW, 0, GUI_ID_TEXT_ITALIAN1, "benvenuto");
      TEXT_SetTextColor(hItem, 0x8ABC0B);
      TEXT_SetFont(hItem, &GUI_Fonteng50);

      hItem = TEXT_CreateEx(10, 130, 115, 25, pMsg->hWin, WM_CF_SHOW, 0, GUI_ID_TEXT_ITALIAN2, "benvenuto");
      ...
      GUI_UC_EnableBIDI(1);
      hItem = TEXT_CreateEx(10, 50, 100, 60, pMsg->hWin, WM_CF_SHOW, 0, GUI_ID_TEXT_ARABIC1, _apStrings[1]);
      TEXT_SetTextColor(hItem, 0x0000FF);
      TEXT_SetFont(hItem, &GUI_Fontarabic60);

      hItem = TEXT_CreateEx(30, 185, 150, 40, pMsg->hWin, WM_CF_SHOW, 0, GUI_ID_TEXT_ARABIC2, _apStrings[1]);
      TEXT_SetTextColor(hItem, 0x2B3C00);
      TEXT_SetFont(hItem, &GUI_Fontarabic40);

      hItem = TEXT_CreateEx(180, 70, 100, 25, pMsg->hWin, WM_CF_SHOW, 0, GUI_ID_TEXT_ARABIC3, _apStrings[1]);
	  ...
	  hItem1 = WM_GetDialogItem(pMsg->hWin, GUI_ID_IMAGE_FRENCH);
          ...
          if (!toggle_drapeau)
          {
            for (step = 1; step < 11; step++)
            {
              if (step < 3)
              {
                WM_MoveTo(hItem, (190 - (step * 18)), 10);
                WM_MoveTo(hItem1, (190 - (step * 18)), 10);
                WM_MoveTo(hItem2, (190 - (step * 18)), 10);
                WM_MoveTo(hItem3, (190 - (step * 18)), 10);
                WM_MoveTo(hItem4, (190 - (step * 18)), 10);
                GUI_Exec();
              }
              else if (step < 5)
              {
          ...
          else
          {
            for (step = 1; step < 10; step++)
            {
              if (step < 3)
              {
                WM_MoveTo(hItem4, (25 + (step * 18)), 10);
                GUI_Exec();
              }
              else if (step < 5)
              {
                WM_MoveTo(hItem4, (25 + (step * 18)), 10);
                WM_MoveTo(hItem3, (25 + (step * 18)), 10);
                GUI_Exec();
              }
              else if (step < 7)
              {
                WM_MoveTo(hItem4, (25 + (step * 18)), 10);
                WM_MoveTo(hItem3, (25 + (step * 18)), 10);
                WM_MoveTo(hItem2, (25 + (step * 18)), 10);
                GUI_Exec();
              }
              else if (step < 9)
              {
                ...
                WM_MoveTo(hItem3, (25 + (step * 18)), 10);
                WM_MoveTo(hItem2, (25 + (step * 18)), 10);
                WM_MoveTo(hItem1, (25 + (step * 18)), 10);
                GUI_Exec();
              }
              else
              {
...
static void _cbDialog(WM_MESSAGE * pMsg) 
{
  ...
  switch (pMsg->MsgId) 
  {
    ...
    case WM_PAINT:
      ...
      if (toggle_flesh == 0)
      {
        GUI_AA_DrawLine(105, 20, 120, 10);
        GUI_AA_DrawLine(120, 10, 135, 20);
      }
      else
      {
        GUI_AA_DrawLine(105, 10, 120, 20);
        GUI_AA_DrawLine(120, 20, 135, 10);
      }
      
      GUI_SetFont(GUI_FONT_13_1);
      GUI_SetColor(GUI_WHITE);
      
      GUI_DispStringAt("STemWin Library supports bidirectional languages", 2, 50);
      GUI_DispStringAt("with different fonts.This example shows how to", 2, 64);
      GUI_DispStringAt("decode text with Unicode characters.", 2, 78);
      GUI_DispStringAt("Press on top right button to select the language", 2, 97);
      GUI_DispStringAt("and highlight the text.", 2, 111);
      
      GUI_DrawBitmap(&bminfo, 205, 10);
...
static void _cbMainDialog(WM_MESSAGE * pMsg) 
{
  ...
  {
    case WM_INIT_DIALOG:
      happli = WM_CreateWindowAsChild(0, 0, 240, 270, hfont, WM_CF_SHOW, _cbDialog_font, 0);
      hinfo = WM_CreateWindowAsChild(0, 270, 240, 450, hfont, WM_CF_SHOW, _cbDialog, 0);
      break;
    default:
      WM_DefaultProc(pMsg);
      break;
